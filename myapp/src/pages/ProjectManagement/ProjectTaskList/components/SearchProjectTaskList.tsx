...
import styles from './SearchProjectTaskList.css'


const SearchProjectTaskList: React.FC = ({handleSearch,afterReset}) => {
  ...
  const handleSearchTaskName = (text) => {
    ...
    handleSearch([text,taskID,creator,taskState]);
    // setSearchText(text);
    setTaskName(text);
  ...
  const handleSearchTaskID = (text) => {
    ...
    handleSearch([taskName,text,creator,taskState]);
    // setInputValue(text);
    setTaskID(text);
  };

  const handleSearchCreator = (text) => {
    console.log('我是handleSearchCreator里的',text)
    handleSearch([taskName,taskID,text,taskState]);
    // setInputValue(text);
    setCreator(text);
  };
  ...
  const onStateChange = (value: string) => {
    ...
    if (text === '全部'){
      handleSearch([taskName,taskID,creator,'']);
      //更新state
      setTaskState('');
    }else{
    ...
    <Col flex="270px">
      <Form.Item name="TaskState" label="任务状态" className={styles.descriptionFont}>
        ...
        >
          ...
          <Option value="creationFailed">创建失败</Option>
        </Select>
      </Form.Item>
    </Col>
...
export default SearchProjectTaskList;
